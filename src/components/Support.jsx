...
import { HelpCircle, Search, Headphones, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function Support({ notices = [], onOpenInquiry }) {
  ...
  const faqs = [
    ...
    },
    {
      ...
      categoryLabel: '회원사유통',
      title: '회원사별 출고 단가 설정 및 수주 주문 접수 시 자동 단가 매칭 방법',
      answer: '거래처 특별단가 관리 메뉴에서 각 회원사별 특별 할인 단가나 거래 조건(할인율, 등급별 단가)을 개별 등록해 주시면 됩니다. 이후 회원사 수주 전표 등록 시 해당 회원사을 선택하는 즉시 해당 품목들의 단가가 지정 단가로 자동 세팅됩니다.',
      views: 1562
    },
    {
      id: 5,
      category: 'distribution',
      categoryLabel: '회원사유통',
      title: '본사와 회원사 간 실시간 재고 공유 및 재고 이동 거래 신청 프로세스',
      answer: '회원사 포털 권한이 있는 사용자는 재고현황 조회 메뉴를 통해 실시간 본사 위탁 창고 재고를 조회할 수 있습니다. 필요 시 재고이동 신청 버튼을 통해 본사 창고에서 회원사 창고로 이송 요청을 발송하고 승인 처리를 진행하게 됩니다.',
      views: 1104
    },
    {
      id: 6,
      category: 'distribution',
      categoryLabel: '회원사유통',
      title: '월별 회원사 매출 마감 및 미수금 정산 현황 확인 경로',
      answer: '매출원장 및 미수금 현황 보고서 메뉴에서 조회 연월을 설정한 뒤 회원사별 거래처 필터를 걸어 조회하시면 당월 매출 마감금액, 입금 수납액, 이월된 잔여 미수금을 한눈에 확인하여 마감 고지서를 발행할 수 있습니다.',
      views: 890
    },
    {
      id: 7,
                ...
                <React.Fragment key={`notice-${notice.id}`}>
                  ...
                  >
                    <td className="px-6 py-4 text-center">
                      <span className="bg-[#f97316]/10 text-[#f97316] border border-[#f97316]/30 px-2 py-0.5 rounded-full text-[0.68rem] font-extrabold whitespace-nowrap inline-block">공지</span>
                    </td>
                    {activeMenu === 'faq' && (
