...
class HomepageErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("Homepage ErrorBoundary caught:", error, info);
  }
  render() {
      ...
      return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white p-6">
          <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
            <h2 className="text-xl font-bold mb-3 text-red-400">일시적인 로딩 오류가 발생했습니다</h2>
            <p className="text-xs text-slate-400 mb-6">아래 버튼을 눌러 새로고침해 주세요.</p>
            <button
